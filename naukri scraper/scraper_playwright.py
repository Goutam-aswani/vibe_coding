...
Uses Playwright browser automation to scrape job listings reliably
"""
import asyncio
...
class NaukriScraper:
    ...
    TIMEOUT = 2000  # 2 second timeout for element operations (default is 30s)
    ...
    async def _init_browser(self):
        ...
        if not self.browser:
            ...
            self.browser = await self.playwright.chromium.launch(
                headless=False,  # Naukri blocks headless browsers
                args=[
    ...
    async def _extract_job_from_card(self, card) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            title = await card.locator('h2 a.title').first.text_content(timeout=self.TIMEOUT)
            title = title.strip() if title else None
            
            # Company name (in row2 > a.comp-name)
            company = await card.locator('a.comp-name').first.text_content(timeout=self.TIMEOUT)
            company = company.strip() if company else None
            
            # Job URL
            job_url = None
            try:
                link = await card.locator('h2 a.title').first.get_attribute('href', timeout=self.TIMEOUT)
                if link:
            ...
            experience = None
            try:
                exp_text = await card.locator('.exp-wrap .expwdth').first.text_content(timeout=self.TIMEOUT)
                if exp_text:
            ...
            salary = None
            try:
                salary_text = await card.locator('.sal-wrap').first.text_content(timeout=self.TIMEOUT)
            ...
            locations = []
            try:
                loc_text = await card.locator('.loc-wrap .locWdth').first.text_content(timeout=self.TIMEOUT)
                if loc_text:
            ...
            description = None
            try:
                desc_text = await card.locator('.job-desc').first.text_content(timeout=self.TIMEOUT)
                if desc_text:
            ...
            try:
                ...
                for tag in tags:
                    skill_text = await tag.text_content(timeout=self.TIMEOUT)
                    if skill_text:
            ...
            posted_date = None
            try:
                date_text = await card.locator('.job-post-day').first.text_content(timeout=self.TIMEOUT)
                if date_text:
            ...
            logo_url = None
            try:
                logo_elem = await card.locator('img.logoImage').first.get_attribute('src', timeout=self.TIMEOUT)
                if logo_elem:
                    logo_url = logo_elem
            except Exception:
                pass
...
if __name__ == "__main__":
    async def test():
        ...
        results = await scraper.search_jobs("python developer", max_pages=1)
